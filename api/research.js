...
let docApiErrorSample = null;

async function fetchDocCount(keyword) {
        const url = `https://naveropenapi.apigw.ntruss.com/search/v1/blog?query=${encodeURIComponent(keyword)}&display=1`;
        const res = await fetch(url, {
                  headers: {
        ...
        if (!res.ok) {
                  if (!docApiErrorSample) {
                              const text = await res.text().catch(() => '');
                              docApiErrorSample = `문서수 API 오류 (${res.status}): ${text.slice(0, 300)}`;
                  }
                  return null;
        }
        const data = await res.json();
